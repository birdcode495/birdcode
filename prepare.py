...
if 'df_gbif_bruto' not in st.session_state or st.session_state['df_gbif_bruto'] is None:
	...
	if st.session_state['analisis_ejecutado'] and st.session_state['df_postgis_resultado'] is not None:
		...
		# 	if 'especie_activa_grafica' not in st.session_state:

		# 		st.session_state['especie_activa_grafica'] = lista_especies.iloc[0]

		# 	st.selectbox('Seleccione el activo biótico a modelar', options = lista_especies, key = 'especie_activa_grafica')

		# 	seleccion_actual = st.session_state['especie_activa_grafica']

		# 	# Isolate the specific dataframe row matching the users selection
		# 	nombre_cientifico_aislado = seleccion_actual.split('(')[1].replace(')', '').strip()
		# 	df_ave_aislada = df_oportunidades[df_oportunidades['nombre_cientifico'] == nombre_cientifico_aislado]

		# 	if not df_ave_aislada.empty:

		# 		ave_seleccionada = df_ave_aislada.iloc[0]
